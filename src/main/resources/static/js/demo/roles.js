...
$(function () {
    ...
    $('#btn-submit').click(function () {
        ...
        req(reqUrl,{rid:rid,rname:rname,rno:rno,comment:comment});
    })
    //添加
    $("#add").click(function () {
        $("#title").html("添加角色");
...
function req(url,obj) {
    $.ajax(url,{
        ...
        success:(res)=>{
            ...
            $("#myModal5").modal("hide");
            //刷新表格
            $('#exampleTableEvents').bootstrapTable('refresh', {url: '/role/listAll'});
        }
    })
...
function del(row) {
    $.ajax("/role/delRole",{
        ...
        data:{rid:row.rid},
        success(){
            swal("删除成功！", "您已经永久删除了"+row.rname+"。", "success");
            setTimeout(function () {
                swal.close();
            },1500)
            //刷新表格
